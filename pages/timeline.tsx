...
const Timeline = () => {
  const [selectedYearTimeline, setSelectedYearTimeline] = useState<YearTimeline | undefined>(
    () => fullTimeline[Math.floor(Math.random() * fullTimeline.length)]
  );
          ...
          {new Array(age).fill(true).map((_, index) => {
            ...
            return (
              ...
              <div
                ...
                className={cx(
                  "flex aspect-square select-none flex-col items-center justify-center rounded-md bg-neutral-100 text-base shadow-md dark:bg-slate-900 dark:text-slate-300 sm:text-lg lg:text-xl ",
                  {
