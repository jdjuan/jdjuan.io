...
const Timeline = () => {
  ...
  return (
    <>
      ...
      <div className='dark:text-slate-300'>Each square is a year of my life:</div>
      <br />
      {/* YEAR BOXES */}
      <div className='grid max-w-sm gap-8 sm:max-w-full sm:grid-cols-2'>
          ...
          {new Array(age).fill(true).map((_, index) => {
            ...
            return (
              ...
              <div
                ...
                className={cx(
                  "flex aspect-square flex-col items-center justify-center rounded-md text-base dark:bg-slate-900 dark:text-slate-300 last:dark:bg-slate-800 sm:text-lg lg:text-xl ",
                  {
                    "cursor-pointer": withEvents,
                    "dark:text-slate-700": notSelectedWithouEvents,
                    "border-0 hover:border-2 hover:dark:border-slate-200": notSelectedWithEvents,
                    "scale-110 border-2 transition dark:border-slate-50 dark:text-slate-50": selected,
                  }
                )}
                key={index}
              >
                ...
                <div className='flex'>
                  {yearTimeline?.highlights.map((_, indexEvent) => (
                    <span
                      key={indexEvent}
                      className={cx(
                        "mr-0.5 aspect-square w-1 rounded-full transition-colors last:mr-0 dark:bg-slate-700 lg:mt-1",
                        {
                          "dark:bg-slate-100": selectedYearTimeline?.year === index,
                        }
                      )}
                    ></span>
                  ))}
        ...
        {selectedYearTimeline !== undefined && (
          <div className='self-start rounded-md p-5 pt-4 dark:bg-slate-900 md:p-7 md:pt-6'>
            ...
            {selectedYearTimeline?.highlights.map((highlight, index) => (
              <div
                key={index}
                className='mb-4 rounded-md p-3 text-xs last:mb-0 dark:bg-slate-800 md:text-sm lg:text-base'
              >
                <highlight.text></highlight.text>
                {highlight.subtext && (
                  <div className='mt-2 text-xs font-light dark:text-slate-400 lg:text-sm'>{highlight.subtext}</div>
                )}
              </div>
            ))}
          </div>
