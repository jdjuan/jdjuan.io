...
export const Books: NextPageWithLayout = () => {
  return (
    <>
      ...
      <div className='grid gap-6 md:gap-5 lg:grid-cols-2 lg:gap-6'>
        {/* BOOK BLOCKS */}
        {booksContent.map(({ author, comment, rating, title, yearRead, isbn, imgUrl, link }) => (
          ...
          <Link
            ...
            key={title}
            target='_blank'
            className='relative box-border flex max-w-sm cursor-pointer rounded-lg border-2 border-transparent border-slate-800 bg-slate-900 p-4 hover:border-slate-50 sm:max-w-full sm:p-5 lg:max-w-full'
          >
            {/* RATING */}
            ...
            <div>
              ...
              <div className='mb-4 font-light'>
                <p className='-mt-1 w-fit text-lg capitalize text-slate-100 sm:text-2xl'>{title}</p>
